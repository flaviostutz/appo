...
## Decision: Use `github.com/golang-jwt/jwt/v5` for bearer-token parsing and signing

**Rationale**:
- No JWT support exists in the current Go module, so the feature needs a pragmatic, low-overhead library.
- `golang-jwt/jwt/v5` is the common Go choice for custom claims and HMAC/PKI verification without pulling in a larger JOSE stack.
- The spec requires custom scope grammars (`i:` and `r:`), which fit naturally into a custom `scope` or `scopes` claim parsed with `MapClaims`.

**Alternatives considered**:
- `lestrrat-go/jwx`: richer JOSE support but unnecessary complexity for the current scope.
- Custom token parsing: rejected because it reinvents security-critical primitives.

## Decision: Start with HS256-signed JWTs and explicit scope-family matching
...
- The feature needs two protected capability families: device operations (`i:account/device/instance/node/attribute:actions`) and registration (`r:account/device`).
- Wildcards must match whole path segments only, which keeps authorization deterministic and auditable.

**Alternatives considered**:
- RS256/JWKS from day one: stronger federation story, but more moving parts than the current monolithic service requires.
- Single mixed scope grammar for everything: rejected because registration authorization is structurally coarser than device-operation authorization.
...
## Decision: Canonical local verification flow remains `make run` in `stutzthings-server`
...
- Planning should therefore include Makefile and bootstrap work to ensure local verification covers bridge ingestion, REST routes, MCP routes, and JWT configuration.
...
- Requiring manual token/bootstrap steps outside Make targets: rejected because it raises the barrier for regression verification.
*** Add File: /Users/flaviostutz/Documents/development/flaviostutz/appo/specs/002-device-ops-api/data-model.md
